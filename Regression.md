...
- 抽出する潜在変数は $2$ 個として、それを表す行列を $Z\in\mathbb{R}^{N\times 2}$ とする。また、この行列を $2$ つの列ベクトルを用いて $Z\equiv(\bm{z}_{1}~\bm{z}_{2})$ のように書く。
- 潜在変数 $Z$ は $X$ を線形変換 $$Z=XW$$ を行って得られるとする。ただし、 $W\in\mathbb{R}^{D\times 2}, W\equiv(\bm{w}_{1}~\bm{w}_{2})$ である。

以上の仮定の下でPLSのモデル式を書き下すと次のようになる。

$$
X=\bm{z}_{1}\bm{p}^{\mathrm{T}}_{1}+\bm{z}_{2}\bm{p}^{\mathrm{T}}_{2}+E
$$

$\bm{p}_{1},\bm{p}_{2}$ はローディング、 $E$ は残差と呼ばれる量である。
