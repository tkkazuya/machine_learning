...
### **尤度（尤度関数）**
- 想定するパラメーターがある値をとる場合に観測している事柄や事象が起こりうる確率。
- パラメータの関数として表されるため、"関数"とも呼ばれる
- 統計的なモデルのパラメータを最も観測値に適合する手法
    - 正規分布だったら平均値・分散
...
- $\theta$は推定したいパラメータ、$x$は観測データ、$L$は尤度関数
- 尤度関数$L$はパラメータ$\theta$が与えられた条件の下で、観測データ$x$が得られる確率を表す。


直感的理解：<https://math-note.xyz/statistics/maximum-likelihood-estimation/>
...
### **最尤推定法**
- 与えられた観測値から、尤度を最大にするパラメータを求めること
- 対数尤度を微分する。

$$
\frac{\partial  \log L(\theta)}{\partial t}=0
$$
尤度について：<https://qiita.com/s-nakagawa2/items/748af2d1df19b25d4d3f>


**交差エントロピーと対数尤度の負値が等しいのは？**
- いずれも２つの確率分布の差異を表そうとしている
- 交差エントロピーは（おそらく）情報理論・機械学習、対数尤度は統計分野での用語？


#### 参考
- クロスエントロピーwiki（英語）：<https://en.wikipedia.org/wiki/Cross-entropy>
- クロスエントロピーwiki（日本語）:
<https://ja.wikipedia.org/wiki/%E4%BA%A4%E5%B7%AE%E3%82%A8%E3%83%B3%E3%83%88%E3%83%AD%E3%83%94%E3%83%BC>
- クロスエントロピー誤差直感理解：<https://qiita.com/kenta1984/items/59a9ef1788e6934fd962>
- 最尤推定とクロスエントロピーの関係：<https://qiita.com/SolKul/items/5f198056295f1e21b867>
$$
q^{N*\frac{i}{N}}*(1-q)^{N*\frac{N-i}{N}}
$$



### **ロジスティック回帰の目的関数**
- 確率的なモデルを元にしているため、尤度関数が用いられる。
    1. 各観測データが属するカテゴリ（0or1）とモデルが予測する確率Pを比較する
    2. その差を最小化するような重み（偏回帰係数）を決定する




### **ロジスティック回帰の特徴重要度が$\exp(w)$で表される理由**
